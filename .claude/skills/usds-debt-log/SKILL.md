---
name: usds-debt-log
description: 【共享】技术债登记 — 记录/更新/还清债务条目 → docs/debt-ledger.md。当用户说「记债/查债」时使用。
context: fork
agent: debt-auditor
---

# 技能：债务登记 (/usds-debt-log)
...
未被记账的债务是危险的。`/usds-debt-log` 让所有债务变得可见、可查、可清偿。

---

## 触发时机 (Triggers)

- 用户运行 `/usds-debt-log`（子命令：`add` / `list` / `resolve` / `report`）
- `/studio-review` 结束时 Debt Auditor 主动追加
- `/vibe-graduate` 前强制清点 sandbox 未记账债务
- `/studio-gate-check` 前生成简报
...
### `/usds-debt-log resolve <id>`
标记某条债务已还清：
- 从"未还清"移入"已还清"章节
- 记录还款日期和方式
- 若通过 `/studio-review` 或 `/studio-gate-check` 触发，自动关联

### `/usds-debt-log report`
生成债务健康简报（不修改 ledger）。
...
| 关注点 | 归属 |
|---|---|
| 代码质量评审 | `/studio-review` |
| 发布合规检查 | `/studio-gate-check` |
| 债务持续追踪 | **`/usds-debt-log`** |
| 微观技术决策记录 | `/usds-micro-adr` |

---

## 协作路径

- 新增高危债务后 → 建议 `/studio-review` 或立即修复
- `/vibe-graduate` 前 → 强制 `/usds-debt-log list` 检查未还账
