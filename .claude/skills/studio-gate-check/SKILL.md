...
description: 【Studio】发布门控 — QA 按发布清单做最后质量验证 → RELEASE-CHECKLIST。当用户说「准备发布/上线前把关」时使用。
context: fork
agent: qa-lead
---

# /studio-gate-check — 质量门控

**代理**: `qa-lead` | **产出**: `docs/reviews/RELEASE-CHECKLIST-xxx.md`

## 参数

- `/studio-gate-check [功能/版本]` — 默认模式：只输出结论摘要 + 报告路径
- `/studio-gate-check --detail [功能/版本]` — 详情模式：在返回摘要中额外展开所有检查项的通过/失败状态
