---
name: vibe-branch
description: 【Vibe】平行探索分叉 — 用独立 git worktree 探索不同技术栈/风格 → .worktrees/<name>/。当用户说「试试完全不同的方向」时使用。
context: main
agent: explorer
---

# 技能：平行探索分叉 (/vibe-branch)
...
如果直接改动当前原型，就丢失了对比证据。`/vibe-branch` 用 **git worktree**
...
### 数量限制
- 同时活跃的分叉建议 ≤ 3 个
- 超过 3 个时，先做一轮 `/vibe-taste-review` 或 `/vibe-check`，淘汰弱势方向

### 合并回主线
分叉胜出后，用 `/vibe-graduate` 晋升（可选择晋升到 `src/` 或替换主 sandbox）。
...
- **不在 git 仓库**：提示先 `git init` 或改用 `/vibe-prototype` 直接新建
...
- 分叉完成 → 在新 worktree 中迭代 `/vibe-prototype`
- 需要对比 → `/vibe-taste-review` 对两个分叉分别评审
- 胜负已分 → `/vibe-graduate` 晋升胜者，删除败者
