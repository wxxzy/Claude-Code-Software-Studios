---
name: usds-mode-switch
description: 【共享】双模态切换 — 初始化/切换 .usds-mode（vibe/studio/hybrid）→ 模式声明文件。当用户说「切换模式/严肃点/放松点」时使用。
context: main
agent: delivery-manager
---

# 技能：模式切换 (/usds-mode-switch)
...
下一步建议: [根据模式给出，如 /vibe-start 或 /studio-discovery]
```
...
- 初始化 vibe → `/vibe-start`
- 初始化 studio → `/studio-discovery` 或 `/studio-setup-stack`
- hybrid → 根据当前任务选择入口
