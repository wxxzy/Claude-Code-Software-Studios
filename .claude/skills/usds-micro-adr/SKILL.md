---
name: usds-micro-adr
description: 【共享】微决策记录 — 一行记录不值得写完整 ADR 的小型技术选型 → docs/arch/micro-adr.md。当用户说「为什么选 X/记一下这个决策」时使用。
context: fork
agent: technical-architect
---

# 技能：微决策记录 (/usds-micro-adr)
...
`/usds-micro-adr` 用**一行一决策**的极简格式追加到 `docs/arch/micro-adr.md`，
保留决策考古的可能性，同时不增加负担。
...
- 用户运行 `/usds-micro-adr "<决策描述>"`
- Explorer / Lead Developer 在做技术选型后主动追加
- `/studio-review` 发现代码内有 `// ai-choice: ...` 注释时提示归档
...
| 架构决策（分层、协议、领域模型） | **adr** | 建议改用 `/studio-arch-design` |
...
- 新条目已追加，格式正确
- 若是架构决策，正确引导用户改用 `/studio-arch-design`
...
| "把 API 从 REST 改为 GraphQL" | **adr**（架构级，用 /studio-arch-design） |
...
| 决策规模 | 归属 |
|---|---|
| 微观选型（一句话说得清） | **`/usds-micro-adr`** |
| 单个系统的架构决策 | `/studio-arch-design` → ADR |
| 跨系统的整体设计 | `/studio-arch-design` → 多个 ADR |
| 债务（不是决策而是妥协） | `/usds-debt-log` |
...
特别是决定 `/vibe-graduate` 晋升时能省大量回忆时间。

---

## 协作路径

- 若判定为架构决策 → 转 `/studio-arch-design`
- 若决策涉及技术债 → 顺便 `/usds-debt-log add` 记账
- `/vibe-graduate` 时 → 建议扫一遍 micro-adr 决定哪些要升级为 ADR
