---
name: usds-cost-report
description: 【共享】成本核算 — 汇总 token/时间/API 费用 → docs/cost-ledger.md。当用户说「花了多少/成本报告」时使用。
context: fork
agent: cost-accountant
---

# 技能：成本报告 (/usds-cost-report)
...
- 用户运行 `/usds-cost-report`（含可选参数 `--period week|month|project`）
...
- "60% token 花在 /studio-discovery，考虑用 /vibe-start 简化"
...
| 关注点 | 归属 |
|---|---|
| Token / 金额 / 时间成本 | **`/usds-cost-report`** |
| 技术债追踪 | `/usds-debt-log` |
| 微观技术决策 | `/usds-micro-adr` |

---

## 协作路径

- 发现异常消耗 → 建议 `/vibe-check` 或 `/vibe-explain-back` 复盘
- 长期项目 → 每月自动建议一次 `/usds-cost-report`
