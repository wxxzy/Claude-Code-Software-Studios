...
任何涉及逻辑变更或文件写入的任务必须经历以下阶段：

1.  **提问 (Question)**: 代理必须先确认需求的模糊点。
2.  **方案 (Options)**: 代理提供 2-3 个方案。**[强制]**：必须调用 `ask_user` 工具提供这些方案的选择按钮。
3.  **决策 (Decision)**: 用户通过交互式组件做出选择。
4.  **草案 (Draft)**: 展示代码片段或文档。**[强制]**：必须调用 `ask_user` 工具并提供 [批准写入]、[修改建议] 或 [拒绝] 按钮。
5.  **批准 (Approval)**: 只有在工具返回“批准”状态后，方可调用写入工具。
